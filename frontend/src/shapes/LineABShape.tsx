import type { Shape, LineABProperties, Line, ObjectProperties, ShapeCreatorInput, ArgumentValue } from '../types';
...
export class LineABShape extends LineBasedShape {
    objectType: ObjectType = ObjectType.LineAB;
    point1: Vector2d;
    point2: Vector2d;
    ...
    protected createClone(): Shape {
        return new LineABShape(this.name, this.description, this.point1, this.point2);
    }
}

export class LineABShapeCreator extends BaseShapeCreator {
    objectType: ObjectType = ObjectType.LineAB;
    ...
    getArgumentValues(properties: ObjectProperties, shapes: Shape[]): ArgumentValue[] {
        const lineABProperties = properties as LineABProperties;
        const point1 = getDefinedOrGridPoint(lineABProperties.point1, shapes);
        const point2 = getDefinedOrGridPoint(lineABProperties.point2, shapes);
    ...
    createShape(input: ShapeCreatorInput): Shape | null {
        ...
        if (points.length == 0) {
            ...
            return new LineABShape(input.objectName, this.getDescription(input), points[0], points[1]);
        }
    }

    protected getDescriptionInner(input: ShapeCreatorInput, argumentStringValues: string[]): string {
        return `${input.objectName} (${argumentStringValues[0]}, ${argumentStringValues[1]})`;
    }
} 